# SCI 데이터 드랍 모델
5G Lena <Master> Branch로 설정  
scratch cttc-error-model로 패킷 손실율 테스트
 ...
 * 
 * 패킷 Loss 요소
 * 1. UE, gNB 간 거리 (움직임 변화)
 * 2. Shadowing (현재 On) - 장애물(건물, 나무 등)에 의한 신호 강도 감소
 * 3. ChannelHelper Period - 이것을 바꾸니 일단 손실율이 증가함
 * 4. LoS/NLoS
 * 5. Fading
 * 6. TxPower
 * 7. MCS
 * 8. Send Packet Period
 * 9. Send Packet Size
 * 10. HARQ Mechanism
 * 
 * 
...
main(int argc, char* argv[])
{
    ...
    double simTime = 50.0; // 50 seconds: to take statistics
    uint32_t pktSize = 500;
    Time udpAppStartTime = MilliSeconds(1000);
    Time packetInterval = MilliSeconds(200);
    Time updateChannelInterval = MilliSeconds(1500);
    bool isUl = true; // uplink on
    ...
    channelHelper->SetChannelConditionModelAttribute("UpdatePeriod", TimeValue(updateChannelInterval)); // 채널 상태 변경 주기
    
    // channelHelper LoS Probability 추가
    // channelHelper->SetChannelConditionModelAttribute("LosProbability", DoubleValue(0.5)); // LoS 확률 50%
    
    channelHelper->SetPathlossAttribute("ShadowingEnabled", BooleanValue(true)); // Shadowing on/off
    ...
    uint32_t totalPackets = packets * ueNodes.GetN();
    ...
    uint64_t totalLost = totalPackets > totalRecv ? totalPackets - totalRecv : 0;
    std::cout << "Sent: " << totalPackets << " Recv: " << totalRecv << " Lost: " << totalLost
          << " pkts, ( " << (static_cast<double>(totalLost) / totalPackets) * 100.0 << " % )"
          << std::endl;
